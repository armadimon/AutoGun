...
public class BinarySpacePartitioner
{
    ...
    public BinarySpacePartitioner(int stageWidth, int stageLength)
    {
        this.rootNode = new RoomNode(new Vector2Int(0, 0), new Vector2Int(stageWidth, stageLength), null, 0);
    }
    ...
    private void SplitTheSpace(RoomNode currentNode, List<RoomNode> listToReturn, int roomLengthMin, int roomWidthMin, Queue<RoomNode> graph)
    {
        ...
        if (line.Orientation == Orientation.Horizontal)
        {
            ...
            node2 = new RoomNode(
                new Vector2Int(currentNode.BottomLeftAreaCorner.x,
                line.Coordinates.y),
                currentNode.TopRightAreaCorner,
                currentNode, currentNode.TreeLayerIndex + 1);
        }
